...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['username', 'password', 'email', 'first_name', 'nickname', 'birthday', 'gender', 'bio']  # 입력 필드
        extra_kwargs = {
    ...
    def create(self, validated_data):
        user = User(
            ...
            bio=validated_data.get('bio', '')  # 선택 입력 필드 (없으면 빈 값으로 처리)
        )
